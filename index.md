...
Step 7:
![Image](step7.png)
Keys I pressed: From the terminal, I typed in `vim ListExamples.java` and pressed `<return>`, then I kept holding `up` until I saw the line reaching 44, then I typed in `i`, then I kept holding `right` until my cursor reaching the space after the 1, then `<delete>`, then typed in 2. Lastly I pressed `<esc>` and typed in `:wq` then pressed `<return>`.
...
Step 9:
![Image](step9f.png)
Keys I pressed: From the terminal, I typed in `git add .` and pressed `<return>`, then `git commit -m "changed from index1 to 2"` and pressed `<return>`, lastly I typed in `git push` and pressed `<return>`.
